...
def ConvertToHpgl( ):
    ...
    MarkerLength = Marker.max(s : s.Perimeter.max(p : p.X)) - Marker.min(s : s.Perimeter.Max(p : p.X))
    MarkerWidth  = Marker.max(s : s.Perimeter.max(p : p.Y)) - Marker.min(s : s.Perimeter.Max(p : p.Y)) 
